...
if __name__ == "__main__":
    ...
    PATH_TO_OUTPUT_FILE: str = os.path.join(PATH_TO_FEATURES_DIR, 'agr_features.pkl')
    ...
    # Filter concept codes only for gender and race
    def excluded_events(event: Event) -> bool:
        # Add logic here to determine success or failure
        if event.code.startswith('Race/') or event.code.startswith('Gender/'):
            return False
        return True

    # Create single time bin
    single_bin = datetime.timedelta(days=365 * 100)
    ...
    age = AgeFeaturizer()
    count = CountFeaturizer(is_ontology_expansion=True, excluded_event_filter=excluded_events, time_bins=[single_bin])
