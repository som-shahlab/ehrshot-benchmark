...
def _increment_days(n_days: int, token: str) -> int:
    ...
    if '[DAY' in token:
        n_days += int(event['code'].replace('[DAY ', '').replace(']', ''))
    elif '[WEEK' in token:
...
def new_pancan_zeroshot_rollouts(timeline: List[str], ontology) -> bool:
    ...
    positive_tokens: Set[int] = get_pancan_codes(ontology)
...
def new_celiac_zeroshot_rollouts(timeline: List[str], ontology) -> bool:
    ...
    positive_tokens: Set[int] = get_celiac_codes(ontology)
    return _time_horizon_eval(timeline, positive_tokens, 365, is_less_than_or_equal=True)
...
def new_lupus_zeroshot_rollouts(timeline: List[str], ontology) -> bool:
    ...
    positive_tokens: Set[int] = get_lupus_codes(ontology)
    return _time_horizon_eval(timeline, positive_tokens, 365, is_less_than_or_equal=True)

def acute_mi_zeroshot_rollouts(timeline: List[str], ontology) -> bool:
    """Return TRUE if acute MI diagnosis occurs in <= 365 days. Otherwise, return FALSE."""
    positive_tokens: Set[int] = get_acutemi_codes(ontology)
    return _time_horizon_eval(timeline, positive_tokens, 365, is_less_than_or_equal=True)
...
def new_hypertension_zeroshot_rollouts(timeline: List[str], ontology) -> bool:
    ...
    positive_tokens: Set[int] = get_hypertension_codes(ontology)
    return _time_horizon_eval(timeline, positive_tokens, 365, is_less_than_or_equal=True)
...
def new_hyperlipidemia_zeroshot_rollouts(timeline: List[str], ontology) -> bool:
    ...
    positive_tokens: Set[int] = get_hyperlipidemia_codes(ontology)
    return _time_horizon_eval(timeline, positive_tokens, 365, is_less_than_or_equal=True)
...
def run_zeroshot_rollout_evals(X_test: np.ndarray, 
                                ...
                                n_generations_per_label: int = 20,
                                ontology = None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n_generations_per_label", type=int, default=20, help="Number of generations to generate for each label. Default to 20 like in ETHOS paper")
...
if __name__ == "__main__":
    ...
    n_generations_per_label: int = args.n_generations_per_label
        ...
        for sub_task_idx, sub_task in enumerate(sub_tasks):
            ...
            if sub_task.startswith('guo') or sub_task.startswith('new_'):
                ...
                scores, metrics = run_zeroshot_rollout_evals(
                    ...
                    n_generations_per_label=n_generations_per_label,
                    ontology=ontology,
            ...
            for score_name, score_value in scores.items():
                results.append({
                    ...
                    'model_hparams' : { 'n_generations_per_label' : n_generations_per_label },
                })
